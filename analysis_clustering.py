"""Run preprocessing -> K selection -> K-Means -> PCA -> persona statistics."""
import argparse
import json
...
def main():
    ...
    result.update({'data_path': str(args.data), 'data_sha256': fingerprint(args.data), 'rows': len(frame)})
    (args.output / 'clustering.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
