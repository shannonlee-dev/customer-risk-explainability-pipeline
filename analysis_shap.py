"""Train/evaluate a Random Forest, then explain global and persona-specific risk."""
import argparse
import json
...
from src.shap_analysis import (create_dependence_plot, create_summary_plot,
                               create_waterfall_plot, explain_positive)
...
def main():
    ...
    global_x = x_test.sample(n=min(args.sample_size, len(x_test)), random_state=SEED)
    ...
    importance = pd.DataFrame({'feature': FEATURES,
                               ...
                               'impurity_importance': model.feature_importances_}).sort_values('mean_abs_shap', ascending=False)
    importance.to_csv(args.output / 'feature_importance.csv', index=False)
    dependencies = list(dict.fromkeys(importance.feature.head(2).tolist() + ['debt_ratio']))
    dependence_stats = []
    directions = []
    for feature in FEATURES:
        ...
        stats = {'feature': feature, 'q25': float(q25), 'q75': float(q75),
        ...
        if feature in dependencies:
            create_dependence_plot(explanation, feature, args.output / f'dependence_{feature}.png')
            dependence_stats.append(stats)
    assignments = pd.read_csv(args.output / 'cluster_assignments.csv').set_index('row_id')
    # Closest held-out row to each full-feature cluster mean; representatives stay out of training.
    scale = x_train.std(ddof=0).replace(0, 1)
    for persona in clusters['personas']:
        members = x_test.loc[assignments.loc[x_test.index, 'cluster'].eq(persona['cluster'])]
        if members.empty:
    ...
    for name, row_id in cases.items():
        ...
        local_results.append({'case': name, 'row_id': row_id,
                              ...
                              'probability': probability, 'base_value': float(exp.base_values),
                              'features': local_x.loc[row_id].to_dict(),
                              'contributions': dict(zip(FEATURES, exp.values.tolist()))})
    pd.DataFrame({'row_id': x_test.index, 'actual_is_overdue': y_test,
                  'probability': model.predict_proba(x_test)[:, 1],
                  'cluster': assignments.loc[x_test.index, 'cluster']}).to_csv(args.output / 'holdout_predictions.csv', index=False)
    pd.DataFrame({'row_id': x_train.index}).to_csv(args.output / 'train_rows.csv', index=False)
    pd.DataFrame(explanation.values, index=global_x.index, columns=FEATURES).rename_axis('row_id').to_csv(args.output / 'global_shap_values.csv')
    result = {'data_sha256': fingerprint(args.data), 'model_source': 'New RandomForest trained on a fixed 75% split; not an imported Mission 23 model.',
              'metrics': metrics, 'global_sample_size': len(global_x),
              'importance': importance.to_dict(orient='records'), 'dependence': dependence_stats,
              'directions': directions,
              'local_cases': local_results,
              'max_additivity_error': float(np.max(np.abs(explanation.base_values + explanation.values.sum(axis=1) - model.predict_proba(global_x)[:, 1])))}
    (args.output / 'shap.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
    print(f'SHAP complete: {len(global_x)} global rows, {len(cases)} local cases; ROC-AUC={metrics["roc_auc"]:.4f}')
